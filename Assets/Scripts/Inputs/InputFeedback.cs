...
public class InputFeedback : MonoBehaviour {
	...
	void Update () {
		...
		if (!blocked) {
			...
			if ((!delaying || Time.time - delayStart < 0.05) && Input.GetKeyDown(pressKey1)) {
			...
			if ((!delaying || Time.time - delayStart < 0.05) && Input.GetKeyDown(pressKey2)) {
